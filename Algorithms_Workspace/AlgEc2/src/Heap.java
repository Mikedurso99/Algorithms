...
public class Heap {
	...
	public static int[] heap; // array heap used as a Heap
	...
	public static void Heapsort() // sort the heap
	{
		
	//complete the method to sort a heap
	//feel free to change the return type, parameters
		
		for (int i = 1; i < n; i++) // n is heap.length
		{
			if (heap[i] < heap[i-1]) // sort heap, has temp as a holder
			{
				int temp = heap[i];
				heap[i] = heap[i-1];
				heap[i-1] = temp;
		...
		for(int i = 0; i<n; i++) {
			System.out.print(heap[i] + " "); // print number with spacing
		}
